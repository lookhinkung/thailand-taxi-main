@php
    $car = App\Models\Car::latest()->limit(6)->get();
@endphp

<style>
    .room-card-two {
        border: 1px solid #e0e0e0;
        margin-bottom: 30px;
        overflow: hidden;
        background: #fff;
        box-shadow: 0px 0px 30px rgba(0, 0, 0, 0.1);
        border-radius: 10px;
    }

    .room-card-img img {
        width: 100%;
        height: auto;
        display: block;
    }

    .room-card-content {
        padding: 20px;
        text-align: left;
    }
    </style>

<div class="room-area pt-100 pb-70 section-bg" style="background-color:#ffffff">
    <div class="container">
        <div class="section-title text-center">
            <span class="sp-color">Fleets</span>
            <h2>Our Fleets</h2>
        </div>
        <div class="row pt-45">

            @foreach ($car as $item)
                <div class="col-lg-4 eq_card">
                    <div class="room-card-two card_">
                        <div class="room-card-img">
                            <a
                                href="{{ route('search_car_details', $item->id . '?check_in=' . old('check_in') . '&check_out=' . old('check_out') . '&persion=' . old('persion')) }}">
                                <img src="{{ asset('upload/carimg/' . $item->image) }}" alt="Images"
                                    style="width:100%; height:auto;">
                            </a>
                        </div>
                        <div class="room-card-content">
                            <h3>
                                <a
                                    href="{{ route('search_car_details', $item->id . '?check_in=' . old('check_in') . '&check_out=' . old('check_out') . '&persion=' . old('persion')) }}">
                                    {{ $item['type']['name'] }}
                                </a>
                            </h3>
                            <span>{{ $item->short_desc }}</span>
                            <p>{{ $item->description }}</p>
                            <ul>
                                <li><i class='bx bx-user'></i> {{ $item->total_passenger }} Passengers</li>
                                <li><i class='bx bx-expand'></i> {{ $item->car_capacity }}</li>
                            </ul>
                            <a href="{{ route('search_car_details', $item->id . '?check_in=' . old('check_in') . '&check_out=' . old('check_out') . '&persion=' . old('persion')) }}"
                                class="book-more-btn">
                                Book Now
                            </a>
                        </div>
                    </div>
                </div>
            @endforeach
        </div>
    </div>
</div>
